import { createDocumentVendor, updateDocumentVendor, updateStockVendor } from "api/items"
import { Fragment, useRef, useState } from 'react'
import { Dialog, Transition } from '@headlessui/react'
import { ExclamationIcon } from '@heroicons/react/outline'
import { useParams } from "react-router-dom"
import LaborVendorForm from "./LaborVendorForm"

const LaborVendorInline = ({ vendor }) => {
  ...
  const onSubmit = data => {
    if (vendor.id) {
      updateStockVendor(id, vendor.id, data)
      .then(res=>{
  ...
  return (
    <div className="divide-y divide-gray-200 bg-white border border-gray-200 rounded-md">
      ...
      <div className="p-2">
        <LaborVendorForm initialValues={vendor} onSubmit={onSubmit} />
      </div>
...
export default LaborVendorInline